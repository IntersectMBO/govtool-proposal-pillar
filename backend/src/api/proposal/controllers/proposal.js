...
module.exports = createCoreController(
	...
	({ strapi }) => ({
		async find(ctx) {
			...
			//////////////////////////////
			/////PROPOSAL SUBMITTED///////////
			const hasPropSubmitedFilter = ctx?.query?.filters['$and']?.find(
				(elem) => elem?.hasOwnProperty('prop_submitted')
			);

			if (hasPropSubmitedFilter) {
				const hasPropSubmitedFilterInSanitize =
					sanitizedQueryParams?.filters['$and']?.some((elem) =>
				...
				if (!hasPropSubmitedFilterInSanitize) {
					if (hasPropSubmitedFilter?.prop_submitted === 'true') {
						sanitizedQueryParams.filters['$and'].push({
							prop_submitted:
								hasPropSubmitedFilter?.prop_submitted,
						});
					} else {
						sanitizedQueryParams.filters['$and'].push({
							$or: [
			...
			if (hasPropIdFilter) {
				...
				if (hasPropIdFilterInSanitize) {
					sanitizedQueryParams.filters['$and'] =
						sanitizedQueryParams.filters['$and']?.filter(
							(elem) => !elem?.hasOwnProperty('prop_id')
						);
			...
			if (hasIsDraftFilter) {
				...
				const hasIsDraftFilterInSanitize =
					sanitizedQueryParams?.filters['$and']?.some((elem) =>
						elem?.hasOwnProperty('is_draft')
					);
				...
				try {
					try {
						...
						if (!proposal?.id) {
							return ctx.badRequest(
								null,
								'Proposal not created or missing ID'
							);
						}
					} catch (error) {
					...
					try {
						...
						console.error(
							'Error creating proposal content:',
							error
						);
