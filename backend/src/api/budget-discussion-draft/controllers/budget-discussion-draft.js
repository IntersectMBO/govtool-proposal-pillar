...
/**
 * budget-discussion-draft controller
 */

const { createCoreController } = require('@strapi/strapi').factories;

module.exports = createCoreController('api::budget-discussion-draft.budget-discussion-draft',
    ({ strapi }) => ({
        async create(ctx) {
            const user = ctx.state.user;
            const data = ctx.request.body.data;
            const newbdg = await strapi.entityService.create("api::budget-discussion-draft.budget-discussion-draft",{data:{creator: user.id, ...data}})
        ...
        async findOne(ctx) {
            ...
            const entity = await strapi.entityService.findOne(
              "api::budget-discussion-draft.budget-discussion-draft",
              id,
              {
                populate: ['creator']
              }
