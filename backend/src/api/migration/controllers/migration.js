...
module.exports = {
				...
				try {
					...
					for (const [key, value] of Object.entries(item)) {
						try {
							...
							if (
								collectionData.hasOwnProperty('be_nationality')
							) {
								let entityValue =
							...
							if (
								collectionData.hasOwnProperty('committee_name')
							) {
								let entityValue =
	...
	async migrateBDProposalsMasterId(ctx) {
		const coreProposals = await strapi.entityService.findMany(
			'api::bd.bd',
			{
				filters: {
					old_ver: null,
		...
		async function propagateMasterId(parentId, coreId) {
			...
			for (const child of children) {
				try {
					if (
						!child.master_id ||
						child.master_id !== coreId.toString()
					) {
					...
					console.log(
						`Error while processing child id ${child?.id}:`,
						error
					);
				}
			}
		...
		for (const proposal of coreProposals) {
			try {
				...
				console.log(
					`Error while processing proposal ${proposal?.id}:`,
					error
				);
			}
		}
	...
	async migrateCommentsAndPollMasterId(ctx) {
		...
		for (const proposal of coreProposals) {
			try {
				...
				const proposalsIDs = allProposalsWithSameMasterId.map(
