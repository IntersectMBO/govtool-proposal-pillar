...
module.exports = {
  ...
  async migrateSubmissions(ctx) {
    ...
    const formattedJson = data?.map((item) => ({
      "api::bd-contact-information.bd-contact-information": {
        ...
        be_country_of_res: item?.["Beneficiary Country of Residence"]?.replace(
          /\u200B/g,
          ""
        ),
      ...
      "api::bd-proposal-ownership.bd-proposal-ownership": {
        ...
        be_country: item?.["Country of Incorporation"]?.replace(/\u200B/g, ""),
    ...
    try {
      for (const item of formattedJson) {
        try {
          ...
          for (const [key, value] of Object.entries(item)) {
            try {
              ...
              if (collectionApiName !== "api::bd.bd") {
                let createdCollectionData = await strapi.entityService.create(
                  // @ts-ignore
                  collectionApiName,
                  {
                    data: collectionData,
          ...
          const createdBd = await strapi.entityService.create("api::bd.bd", {
            data: {
              ...bdData,
              ...bdRelationIds.reduce((acc, obj) => ({ ...acc, ...obj }), {}),
            },
          });
  ...
  async migrateAmounts(ctx) {
    const allBdCostings = await strapi.db
      .query("api::bd-costing.bd-costing")
      .findMany({
        select: [
          "id",
          "ada_amount",
    ...
    let unfinishedCostings = [];
    for (let bdCost of allBdCostings) {
      try {
        let updatedEntry = await strapi.entityService.update(
          ...
          bdCost?.id,
          {
            data: {
              ada_amount_clone:
                +bdCost?.ada_amount?.toString()?.replace(",", ".") || 0,
              amount_in_preferred_currency_clone:
        ...
        if (!updatedEntry) {
          unfinishedCostings.push(bdCost?.id);
        }
      } catch (error) {
        console.log(error);
        unfinishedCostings.push(bdCost?.id);
    ...
    let returnValue =
      unfinishedCostings?.length > 0
        ? { success: "partial", unfinishedCostings: unfinishedCostings }
        : { success: true };

    return returnValue;
  },

  async migrateLinkProposalToProposalContent(ctx) {
    const allProposalContent = await strapi.db
    ...
    for (const item of allProposalContent) {
      try {
        await strapi.entityService.update(
          ...
          {
            data: {
              proposal: parseInt(item.proposal_id),
            },
            populate: ["proposal"],
          }
        );
